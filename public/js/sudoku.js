...
function selected (evt) {
    ...
    if (data[evt.currentTarget.coord_x][evt.currentTarget.coord_y]==0) {
        document.querySelector(`.el${evt.currentTarget.coord_x}${evt.currentTarget.coord_y}`).style.backgroundColor="grey";
    } else {
        selected_cell="";
    }
}
...
function enterNumber (event) {
    if (selected_cell!="" && ["1","2","3","4","5","6","7","8","9"].includes(event.key) && document.querySelector(`.el${selected_cell}`).style.backgroundColor=="grey") {
...
function solver (selected_cell) {
    ...
    for (let number of valid_numbers) {
        ...
        setTimeout(() => {updateBoard(number, "grey", selected_cell)}, timer);
        step_count++;
        timer+=10;
        if (solver(blankCell())) {
            return true;
